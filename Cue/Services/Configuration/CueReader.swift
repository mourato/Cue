...
struct CueConfigurationReader {
    let document: SimpleTOMLDocument
    var issues: [CueConfigurationIssue] = []
    ...
    mutating func error(_ message: String) {
        issues.append(CueConfigurationIssue(severity: .error, message: message))
    }

    mutating func warning(_ message: String) {
        issues.append(CueConfigurationIssue(severity: .warning, message: message))
    }
}
