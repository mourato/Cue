...
@MainActor
final class CueConfigurationSyncCoordinator: ObservableObject {
    ...
    static let shared = CueConfigurationSyncCoordinator()

    @Published private(set) var status: Status = .idle

    private let notificationCenter: NotificationCenter
    private let debounceInterval: TimeInterval
    private let syncManagedConfigIfSafe: @MainActor () throws -> CueConfigurationSyncResult
    private let syncManagedConfigIfSafeInBackground: @MainActor () async throws -> CueConfigurationSyncResult
    ...
    init(
        ...
        currentSettingsSignature = {
            CueConfigurationAutoImporter.contentSignature(for: service.exportTOML())
        }
    }
    ...
    @discardableResult
    func flushPendingSync(reason: Reason) throws -> CueConfigurationSyncResult? {
    ...
    @discardableResult
    func syncNow(reason: Reason = .manual) throws -> CueConfigurationSyncResult {
        cancelPendingSync()
        let sequence = beginSync()
        var syncResult: CueConfigurationSyncResult?
        defer {
    ...
    @discardableResult
    func syncNowInBackground(reason: Reason = .manual) async throws -> CueConfigurationSyncResult {
        cancelPendingSync()
        let sequence = beginSync()
    ...
    private func updateStatus(for result: CueConfigurationSyncResult, sequence: Int) {
    ...
    private func finishSyncing(after result: CueConfigurationSyncResult?, sequence _: Int) {
    ...
    private func log(_ result: CueConfigurationSyncResult, reason: Reason) {
        DiagnosticLogger.shared.log(
            .debug,
            .preferences,
