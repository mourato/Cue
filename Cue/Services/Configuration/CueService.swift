...
@MainActor
final class CueConfigurationService {
    ...
    var suggestedConfigURL: URL {
        CueConfigurationPaths.suggestedConfigURL
    }

    var suggestedConfigDirectoryURL: URL {
        CueConfigurationPaths.suggestedConfigDirectoryURL
    }
    ...
    var suggestedConfigRootDirectoryURL: URL {
        CueConfigurationPaths.userHomeDirectory
    }
    ...
    func exportTOML() -> String {
        CueConfigurationExporter.exportTOML()
    }
    ...
    static func syncDecision(
        ...
        if CueConfigurationAutoImporter.isCurrentFileApplied(fileSource, defaults: defaults) {
    ...
    func importTOML(_ source: String) -> CueConfigurationImportResult {
        CueConfigurationImporter.importTOML(source)
    }

    func `import`(from url: URL) throws -> CueConfigurationImportResult {
    ...
    func importBackupReplacingManagedConfig(
        ...
        guard !validationIssues.contains(where: { $0.severity == .error }) else {
            return CueConfigurationImportResult(appliedChangeCount: 0, issues: validationIssues)
        }
    ...
    func restoreDefaultsReplacingManagedConfig() throws -> CueConfigurationImportResult {
    ...
    func prepareManagedConfigForOpening(at url: URL? = nil) throws -> CueConfigurationSyncResult {
        try syncManagedConfigIfSafe(at: url)
    }

    func syncManagedConfigIfSafe(at url: URL? = nil) throws -> CueConfigurationSyncResult {
        if url == nil, needsUserSelectedConfigAccess {
            return CueConfigurationSyncResult(status: .permissionRequired, fileURL: resolvedConfigFileURL)
        }
    ...
    func syncManagedConfigIfSafeInBackground(at url: URL? = nil) async throws -> CueConfigurationSyncResult {
        if url == nil, needsUserSelectedConfigAccess {
            return CueConfigurationSyncResult(status: .permissionRequired, fileURL: resolvedConfigFileURL)
        }
        ...
        try Self.managedConfigFileQueue.sync {
            ...
            if let expectedFileSignature {
                guard fileManager.fileExists(atPath: targetURL.path) else {
                    throw CueConfigurationSyncError.fileChangedSinceConfirmation
                }
                let currentFileSource = try String(contentsOf: targetURL, encoding: .utf8)
                let currentFileSignature = CueConfigurationAutoImporter.contentSignature(for: currentFileSource)
    ...
    private func markCurrentFileAppliedIfLatest(_ source: String, operationID: Int) {
        ...
        CueConfigurationAutoImporter.markCurrentFileApplied(source, defaults: defaults)
    }
    ...
    private nonisolated static func syncManagedConfigFile(
        ...
        let currentSignature = CueConfigurationAutoImporter.contentSignature(for: currentSource)
        guard fileManager.fileExists(atPath: fileURL.path) else {
            try currentSource.write(to: fileURL, atomically: true, encoding: .utf8)
            return ManagedConfigFileSyncOutcome(
                result: CueConfigurationSyncResult(
                    status: .synced,
                    fileURL: fileURL,
        ...
        let fileSignature = CueConfigurationAutoImporter.contentSignature(for: fileSource)
        if fileSource == currentSource {
            return ManagedConfigFileSyncOutcome(
                result: CueConfigurationSyncResult(
