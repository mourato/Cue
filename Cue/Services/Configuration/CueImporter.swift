...
@MainActor
enum CueConfigurationImporter {
    private struct PreparedImport {
        let issues: [CueConfigurationIssue]
        let mutations: [() -> Void]
    }

    static func validateTOML(_ source: String, defaults: UserDefaults = .standard) -> [CueConfigurationIssue] {
        prepareImport(source, defaults: defaults).issues
    }

    static func importTOML(_ source: String, defaults: UserDefaults = .standard) -> CueConfigurationImportResult {
        ...
        guard !preparedImport.issues.contains(where: { $0.severity == .error }) else {
            return CueConfigurationImportResult(appliedChangeCount: 0, issues: preparedImport.issues)
        }
    ...
    private static func prepareImport(_ source: String, defaults: UserDefaults) -> PreparedImport {
        ...
        var reader = CueConfigurationReader(document: document)
        var mutations: [() -> Void] = []
    ...
    private static func validateSchema(_ reader: inout CueConfigurationReader) {
    ...
    private static func expandedPath(_ path: String) -> String {
        CueConfigurationPaths.expandedUserPath(path)
    }
}
