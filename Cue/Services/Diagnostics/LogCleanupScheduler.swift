...
final class LogCleanupScheduler {
    ...
    private func performCleanup() {
        ...
        let logDirectoryURL = DiagnosticLogger.shared.logDirectoryURL

        // The worker receives only immutable snapshots and never crosses back into the MainActor.
        DispatchQueue.global(qos: .utility).async { [retentionDays, logDirectoryURL] in
            Self.deleteOldFiles(logDirectoryURL: logDirectoryURL, retentionDays: retentionDays)
        }
    }

    /// Delete daily log files older than the configured retention window.
    private nonisolated static func deleteOldFiles(logDirectoryURL: URL, retentionDays: Int) {
