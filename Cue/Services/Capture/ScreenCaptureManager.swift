...
@MainActor
final class ShareableContentBox {
...
typealias ShareableContentPrefetchTask = Task<ShareableContentBox, Error>
...
final class ScreenCaptureManager: ObservableObject {
    ...
    private nonisolated static func alphaByteOffset(for image: CGImage) -> Int? {
        ...
        guard let alphaInfo = CGImageAlphaInfo(rawValue: image.bitmapInfo.rawValue & CGBitmapInfo.alphaInfoMask
            .rawValue)
        else {
            return nil
        }
    ...
    private func loadShareableContent(
        ...
        if let prefetchedContentTask {
            do {
                let content = try await prefetchedContentTask.value.value
        ...
        if let cachedTask = prefetchShareableContent(includeDesktopWindows: includeDesktopWindows) {
            do {
                let content = try await cachedTask.value.value
        ...
        do {
            let content = try await refreshedTask.value.value
    ...
    private func makeShareableContentPrefetchTask(includeDesktopWindows: Bool) -> ShareableContentPrefetchTask {
        Task(priority: .userInitiated) {
            try await ShareableContentBox(self.fetchShareableContent(includeDesktopWindows: includeDesktopWindows))
        }
    }
