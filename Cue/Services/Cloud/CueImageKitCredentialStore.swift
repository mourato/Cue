...
enum CueImageKitCredentialError: LocalizedError, Equatable {
    case emptyKey
...
@MainActor
final class CueImageKitCredentialStore: ObservableObject {
    static let shared = CueImageKitCredentialStore()

    @Published private(set) var revision = UUID()
    /// Cached without unlocking Keychain. See `CueImgBBCredentialStore.isConfigured`.
    ...
    func save(privateKey: String) throws {
        ...
        guard !trimmed.isEmpty else { throw CueImageKitCredentialError.emptyKey }
        do {
            try keychain.upsert(value: trimmed)
            defaults.set(true, forKey: PreferencesKeys.imageKitCredentialConfigured)
            publishChange()
        } catch {
            throw CueImageKitCredentialError.keychainWriteFailed(error.localizedDescription)
        }
    }
