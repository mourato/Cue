...
struct CueNoteEditorCanvasOverlay: View {
    ...
    @State private var draftNote: CueVisualNote?
    @State private var openingSnapshot: CueVisualNote?
    @State private var panelPlacement = CueNoteEditorPanelPlacement()
    ...
    private func reconcilePlacementIfNeeded(
        ...
        let panelChanged = lastTrackedPanelSize.map {
            !CueNoteGeometry.sizesAreEffectivelyEqual($0, panelSize)
        } ?? true
        let hostChanged = lastTrackedHostSize.map {
            !CueNoteGeometry.sizesAreEffectivelyEqual($0, hostSize)
        } ?? true
    ...
    private func draftTextBinding(for note: CueVisualNote) -> Binding<String> {
        Binding(
    ...
    private func draftColorBinding(for note: CueVisualNote) -> Binding<RGBAColor> {
        Binding(
    ...
    private func draftAreaStyleBinding(for note: CueVisualNote) -> Binding<CueAreaStyle> {
        Binding(
    ...
    private func draftAreaStrokeWidthBinding(for note: CueVisualNote) -> Binding<CGFloat> {
        Binding(
    ...
    private func activeDraft(for note: CueVisualNote) -> CueVisualNote {
    ...
    private func ensureDraftSeeded(from note: CueVisualNote) {
    ...
    private func updateDraft(from note: CueVisualNote, mutate: (inout CueVisualNote) -> Void) {
        ensureDraftSeeded(from: note)
    ...
    private func applyLiveAppearance(
        ...
        updateDraft(from: note) { updated in
            ...
            if let areaStrokeWidth {
                updated.areaStrokeWidth = CueVisualNote.clampedAreaStrokeWidth(areaStrokeWidth)
            }
        }

        guard let updated = draftNote else { return }
        var liveNote = updated
        if let stateNote = state.cueNotes.first(where: { $0.id == updated.id }) {
