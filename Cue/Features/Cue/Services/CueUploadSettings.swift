...
struct CueUploadEncodingSettings: Equatable, Sendable {
    ...
    let imageFormat: CueUploadImageFormat
    ...
    static func current(defaults: UserDefaults = .standard) -> Self {
        ...
        let format = defaults.string(forKey: PreferencesKeys.uploadImageFormat)
            .flatMap(CueUploadImageFormat.init(rawValue:))
            ?? .webp
...
struct CueEncodedImage: Equatable, Sendable {
    let data: Data
    let fileExtension: String
    let contentType: String
}

struct CuePreparedUpload: Sendable {
    let url: URL
...
enum CueUploadEncodingError: Error {
...
nonisolated enum CueUploadImageEncoder {
    ...
    static func encode(imageData: Data, settings: CueUploadEncodingSettings) throws -> CueEncodedImage {
        ...
        guard let image else { throw CueUploadEncodingError.failedToCreateBitmap }

        return try encode(rasterImage: image, settings: settings)
    }

    static func encode(image: CGImage, settings: CueUploadEncodingSettings) throws -> CueEncodedImage {
    ...
    private static func encode(
        ...
        let format: CueUploadImageFormat = if settings.optimizeImages {
        ...
        guard let encodedData, !encodedData.isEmpty else {
            throw CueUploadEncodingError.failedToEncode
        }
