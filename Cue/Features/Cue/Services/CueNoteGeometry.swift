...
nonisolated enum CueNoteGeometry {
    ...
    static func pinAnchor(for target: CueNoteTarget) -> CGPoint {
        target.pinCenter
    }
    ...
    static func displayNumber(for note: CueVisualNote, in notes: [CueVisualNote]) -> Int {
    ...
    static func nextCreationOrder(in notes: [CueVisualNote]) -> Int {
    ...
    static func orderedRenderableNotes(_ notes: [CueVisualNote]) -> [CueVisualNote] {
        notes
    ...
    static func hitTest(note: CueVisualNote, at point: CGPoint) -> Bool {
        selectionBounds(for: note)
            .insetBy(dx: -6, dy: -6)
            .contains(point)
    }

    static func selectionBounds(for note: CueVisualNote) -> CGRect {
    ...
    static func canvasDisplayNumber(for noteID: UUID, in notes: [CueVisualNote]) -> Int? {
