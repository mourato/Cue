...
//        Color(nsColor: .textBackgroundColor)
        ...
        .onKeyPress { keyPress in
            handleCanvasKeyPress(keyPress)
        }
        .onAppear {
            isCanvasFocused = true
        }
        .onChange(of: state.editingTextAnnotationId) { editingID in
            if editingID == nil {
                isCanvasFocused = true
            }
        }
    ...
    /// Route canvas key presses through SwiftUI focus. Return `.ignored` for
    /// non-tool keys so Delete, Space, arrows, and system shortcuts keep working.
