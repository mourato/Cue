...
final class AnnotateExporter {
    ...
    static func saveToOriginal(state: AnnotateState) -> Bool {
        ...
        DiagnosticLogger.shared.log(
            ...
            .annotate,
            "Save to original",
            context: ["file": sourceURL.lastPathComponent]
        )
    ...
    static func renderFinalImage(state: AnnotateState) -> NSImage? {
        ...
        if snapshot.editorMode == .mockup {
            ...
            return composeNotinhasIfNeeded(
                compositeMockupImage(flatImage: flatImage, snapshot: snapshot),
                snapshot: snapshot
            )
        }
    ...
    nonisolated static func renderFlatFinalImage(snapshot: AnnotateRenderSnapshot) -> NSImage? {
        ...
        let padding = snapshot.isCombineMode ? snapshot
            .padding : (snapshot.backgroundStyle != .none ? snapshot.padding : 0)
    ...
    private nonisolated static func offsetAnnotation(
        _ annotation: AnnotationItem,
        by padding: CGFloat
    ) -> AnnotationItem {
        var result = annotation
            ...
            color
                ...
                .overlay(
                    (snapshot.isBlurredBackgroundEffectActive ? snapshot.blurredBackgroundEffect.tintColor : .clear)
                        .opacity(snapshot.isBlurredBackgroundEffectActive ? snapshot.blurredBackgroundEffect
                            .tintOpacity : 0)
                )
        case let .wallpaper(url):
