...
Notinhas is a personal local macOS visual-handoff tool with no server budget
...
- Keep provider adapters thin and Foundation-only. Do not add a Notinhas
...
request can still create a remote object; Notinhas does not provide automatic
cleanup or retry. Existing ImgBB credentials and persisted action identifiers
remain compatible.

## Revisit triggers

Reconsider this boundary if Notinhas is distributed to other users, private
