...
- Lifecycle contract: a recording is either finalized and validated before entering Quick Access/History, explicitly cancelled, or preserved as a recoverable partial. Each processing session has an atomic `recording-manifest.json` with a UUID, relative writer filename, checkpoint state, and finalization flag. Startup cleanup never deletes processing sessions before validation or explicit retention/promotion; paths and UUIDs are never shown to users. The current writer remains a single non-fragmented file because fragment rotation/stitching is not validated for the active codec/container pipeline.
