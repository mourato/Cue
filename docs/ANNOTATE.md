...
- Quick properties bar (`AnnotateQuickPropertiesBar`): context controls — primary color, text background, blur type, arrow style/bend/heads, watermark text/style/opacity/rotation, stroke/size and shape-style popovers (discrete `AnnotationStrokeWidth` + fill styles), font size, corner radius. Modes `hidden / toolDefaults / selectedItem`. The island sizes to content on one centered row (no wrap); labeled slider and text controls size intrinsically so labels do not overlap fields.
