...
| `uploadToImgBB` | Manual upload of an encoded image, GIF, or supported video through the selected provider and public-link copy; the raw identifier remains for compatibility. Cloudflare accepts images/GIFs/videos up to the fixed 95 MiB target and uses the same local MP4/H.264 optimization popover. Requires the selected provider's Keychain credential. |
