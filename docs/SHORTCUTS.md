...
Global keyboard shortcuts, in-overlay capture shortcuts, Annotate editor shortcuts, conflict detection, the cheat-sheet overlay, and the `cue://` deep-link route table.

Verified against `Cue/Services/Shortcuts/`, `Cue/Features/Shortcuts/`, `Cue/Features/Annotate/Services/AnnotateShortcutManager.swift`, `Cue/App/CueDeepLinkHandler.swift`, `Cue/Services/Capture/CaptureOverlayShortcutSettings.swift` at HEAD (`v1.30.0-beta.4`).
...
`AnnotateShortcutManager` (`Cue/Features/Annotate/Services/AnnotateShortcutManager.swift`):
...
`ShortcutValidationService` (`Cue/Services/Shortcuts/ShortcutValidationService.swift`):
...
- `ShortcutOverlayManager` (`Cue/Features/Shortcuts/ShortcutOverlayManager.swift`) — full-screen borderless `NSPanel` (`.screenSaver` level, joins all spaces), content from `ShortcutOverlayContentBuilder.buildSections()` (`Cue/Features/Shortcuts/ShortcutOverlayModels.swift`).
- Open/toggle: ⇧⌘K, menu bar → Keyboard Shortcuts, or `cue://show/shortcuts`. Blocked while recording (`RecordingCoordinator.shared.isActive` guard).
- Esc closes (local + global monitors); "Open Settings" deep-links to Settings → Shortcuts.

## URL scheme automation

Scheme: `cue://` (registered in `Cue/Resources/Info.plist` `CFBundleURLTypes`).
...
Dispatch: AppleEvent `kAEGetURL` → `AppDelegate` (queued pre-launch) → `AppCoordinator.handleDeepLink` → `CueDeepLinkHandler` (`Cue/App/CueDeepLinkHandler.swift`); routes parsed by `CueDeepLinkAction.init?(url:)`.
...
| `cue://capture/area-annotate` | Capture area → Annotate |
| `cue://capture/scrolling` | Scrolling capture |
| `cue://capture/ocr` | OCR capture |
| `cue://capture/smart-element` | Smart Element capture |
| `cue://capture/object-cutout` | Object cutout |
| `cue://record/screen` | Start screen recording |
| `cue://record/application` | Application-window recording |
| `cue://open/annotate` | Open empty Annotate editor |
| `cue://open/combine` | Combine images (see params below) |
...
- Settings tabs: `general`, `capture`, `annotate`, `quick-access`, `history`, `shortcuts`, `permissions`, `uploads`, `advanced`. Also accepted as path form (`cue://settings/capture`).
