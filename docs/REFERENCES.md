...
| [Reframed](https://github.com/jkuri/Reframed) | UI/UX; mesmo domínio; engenharia | Refino de área com 8 handles; cursores diagonais via API privada AppKit alinhados aos H/V de sistema; gravação/replay de cursor; editor com trim/timeline. | Unificar cursores de resize da seleção (AIO/área) com aparência de sistema em vez de misturar arte custom só nos cantos. | [MIT](https://opensource.org/license/mit). Inspiração; eventual reutilização só com avisos exigidos pela licença. | Sim; consulta local |
