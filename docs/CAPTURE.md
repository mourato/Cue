...
- Fullscreen mode hides area refinement. Each mode exits the HUD and calls its existing capture entry point: Window opens application-window selection, while Scrolling starts its existing flow. Recording passes the refined rect directly to the recording toolbar, so All-In-One does not open a second area selection; if no rect exists yet, it keeps the normal recording selection fallback.
