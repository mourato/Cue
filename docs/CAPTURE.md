...
- Fullscreen resolves the active display from `ScreenUtility.activeDisplayID()` when triggered, then runs through `ScreenCaptureManager.captureAllDisplays(targetDisplayIDs:)` so `Cmd+Shift+3` captures only the screen the user is interacting with via ScreenCaptureKit.
...
- `Cmd+Shift+4` area capture starts in manual region selection; press `Space` to switch to application-window selection. Application-window capture is also available from the menu, Active Window shortcut, or All-In-One → Window (HUD child key, default `A`).
...
- Mode buttons are direct capture actions configured in Preferences → Capture → All-In-One Modes. Enabled modes appear in the saved order; Recording appears only when the Video module is enabled and may occupy any saved position. Active Window captures the focused window directly; Object Cutout and Smart Element hand off to their existing selection and accessibility-driven flows after the HUD closes.
...
- The All-In-One mode strip does not show a persistent selected tint. The active mode remains an internal and accessibility state so `Return` can dispatch it and the dimensions bar can follow it.
...
- Fullscreen mode hides area refinement. Each mode exits the HUD and calls its existing capture entry point: Window opens application-window selection, Active Window captures the focused window directly, Object Cutout starts its area selection, Smart Element starts its accessibility-driven overlay, and Scrolling starts its existing flow. Recording passes the refined rect directly to the recording toolbar, so All-In-One does not open a second area selection; if no rect exists yet, it keeps the normal recording selection fallback.
...
- When **Freeze screen** is enabled, All-In-One prepares one shared all-display `FrozenAreaCaptureSession` before HUD/selection UI, uses frozen backdrops for the first drag and last-rectangle refinement (`AllInOneFrozenBackdropHost` during refinement), and reuses that session for Area, Capture Markup, and OCR dispatch. Timer keeps frozen selection UI but prepares a fresh all-display session when the delay fires. Window inherits the normal multi-display frozen area/application path. Scrolling, Fullscreen, and Recording remain outside this screenshot-area preference; Recording still receives the selected rect as an in-memory handoff.
- Every mode handoff tears down the All-In-One HUD, refinement callbacks, and any first-drag selection before dispatching. No stale All-In-One or area-selection session remains active during the next flow, which makes Escape/re-entry safe.
...
- Interaction modes (`AreaSelectionInteractionMode`): `.manualRegion` and `.applicationWindow`. During screenshot and recording area selection, `Space` switches from manual region to application-window mode; it does not move an active rectangle. Results flow as `AreaSelectionResult{target: .rect|.window, displayID, mode, displayIDs, spansMultipleDisplays}`.
...
- Object cutout (`ForegroundCutoutService`, Vision `VNGenerateForegroundInstanceMaskRequest`, all subject instances masked on the full canvas) is always available. Fully on-device.
