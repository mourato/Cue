...
@MainActor
final class CueConfigurationSyncCoordinatorTests: XCTestCase {
    ...
    func testSyncNowReportsConflictWithoutConfirmedOverwrite() throws {
        ...
        let coordinator = makeCoordinator(
            sync: {
                syncCount += 1
                return CueConfigurationSyncResult(status: .needsConfirmation, fileURL: fileURL)
            },
    ...
    private func makeCoordinator(
        ...
        sync: @escaping @MainActor () throws -> CueConfigurationSyncResult = {
            CueConfigurationSyncResult(status: .alreadyCurrent, fileURL: URL(fileURLWithPath: "/tmp/config.toml"))
        },
        backgroundSync: (@MainActor () async throws -> CueConfigurationSyncResult)? = nil,
