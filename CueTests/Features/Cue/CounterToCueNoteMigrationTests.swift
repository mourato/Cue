//
//  CounterToCueNoteMigrationTests.swift
//  NotinhasTests
//
...
final class CounterToCueNoteMigrationTests: XCTestCase {
    func testMigrate_noCounters_isIdempotent() {
        ...
        let result = CounterToCueNoteMigration.migrate(annotations: annotations, cueNotes: [existing])

        XCTAssertFalse(result.didMigrate)
        XCTAssertEqual(result.annotations, annotations)
        XCTAssertEqual(result.cueNotes, [existing])
    }
    ...
    func testMigrate_convertsCountersAndAppendsAfterExistingNotes() {
        ...
        XCTAssertEqual(result.cueNotes.count, 3)
        XCTAssertEqual(result.cueNotes[0].creationOrder, 3)

        let migratedFirst = result.cueNotes[1]
        ...
        let migratedSecond = result.cueNotes[2]
    ...
    func testMigrate_secondRun_isIdempotent() {
        ...
        XCTAssertEqual(second.cueNotes, first.cueNotes)
    }
    ...
    func testAnnotateState_migrateLegacyCountersToNotinhasIfNeeded() {
        ...
        XCTAssertEqual(state.cueNotes.count, 1)
        XCTAssertEqual(state.cueNotes[0].creationOrder, 1)
        XCTAssertTrue(state.hasUnsavedChanges)
    }

    private func makeNote(order: Int, center: CGPoint, color: RGBAColor? = nil) -> CueVisualNote {
        CueVisualNote(
            target: .point(center),
