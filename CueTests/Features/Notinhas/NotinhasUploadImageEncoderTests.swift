...
@MainActor
final class NotinhasUploadImageEncoderTests: XCTestCase {
    func testEncodeLimitsPhysicalPixelDimension() throws {
        ...
        let settings = NotinhasUploadEncodingSettings(
            optimizeImages: true,
            imageFormat: .png,
        ...
        let encoded = try NotinhasUploadImageEncoder.encode(imageData: imageData, settings: settings)
    ...
    func testPrepareWritesDerivativeWithoutChangingOriginal() async throws {
        ...
        let prepared = try await NotinhasUploadImageEncoder.prepare(fileURL: sourceURL, settings: settings)
