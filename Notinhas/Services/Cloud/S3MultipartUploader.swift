...
final class S3MultipartUploader {
            ...
            for partNumber in 1 ... totalParts {
                ...
                guard let partData = try fileHandle.read(upToCount: partSize), !partData.isEmpty else {
                    throw CloudError.uploadFailed(
                        statusCode: 500,
                        message: "Failed to read part data at offset \(offset)",
                    )
                }
                ...
                while retryCount < 3, partETag == nil {
                    do {
                        ...
                        logger
                            .warning(
                                "Failed to upload part \(partNumber) (attempt \(retryCount)/3): \(error.localizedDescription)",
                            )
                        if retryCount >= 3 {
                            throw error
                        }
    ...
    private func uploadPart(
        ...
        let url =
            URL(string: "\(endpoint.absoluteString)/\(bucket)/\(key)?partNumber=\(partNumber)&uploadId=\(uploadId)")!
        var request = URLRequest(url: url)
    ...
    private func completeMultipartUpload(
        ...
        if let customDomain = await CloudManager.shared.cachedConfiguration?.customDomain, !customDomain.isEmpty {
