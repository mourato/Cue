...
final class CloudManager: ObservableObject {
    ...
    func exportTransferPayload() throws -> CloudCredentialTransferPayload {
        guard let configuration = loadConfiguration(),
            ...
            DiagnosticLogger.shared.log(
                .warning,
                .cloud,
                "Cloud credential export failed because cloud is not configured",
            )
    ...
    private func rollbackCredentialWrite(
        ...
        restoreCredential(
            item: accessItem,
            snapshot: accessKeySnapshot,
            context: "saveConfiguration.rollback.accessKey",
        )
        restoreCredential(
            item: secretItem,
            snapshot: secretKeySnapshot,
            context: "saveConfiguration.rollback.secretKey",
        )
    ...
    func deleteByKey(key: String) async throws {
        guard let provider = createProvider() else {
            DiagnosticLogger.shared.log(
                .warning,
                .cloud,
                "Cloud delete-by-key requested while provider is not configured",
            )
            throw CloudError.notConfigured
        }
        do {
    ...
    func deleteAllFromCloud(records: [CloudUploadRecord]) async throws {
        guard let provider = createProvider() else {
            DiagnosticLogger.shared.log(
                .warning,
                .cloud,
                "Bulk cloud delete requested while provider is not configured",
            )
            throw CloudError.notConfigured
        }
