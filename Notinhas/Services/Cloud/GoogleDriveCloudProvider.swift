...
final class GoogleDriveCloudProvider: CloudProvider {
    ...
    func setExpiration(days: Int) async throws {
        ...
        logger
            .info("Google Drive setExpiration called (\(days) days). Custom lifecycle rules are unsupported, skipping.")
    }
    ...
    private func ensureValidAccessToken() async throws -> String {
        ...
        do {
            ...
            if tokens.refreshToken != refreshToken {
                try? await CloudManager.shared.saveGoogleRefreshToken(tokens.refreshToken)
            }
    ...
    private func ensureFolder(accessToken: String) async throws -> String {
        // Keep this immutable configuration snapshot explicit before the first await.
        ...
        // 3. Create new folder
        logger.info("Creating folder '\(folderName)' in Google Drive")
    ...
    private func performMultipartUpload(
        ...
        guard httpResponse.statusCode == 200 || httpResponse.statusCode == 201 else {
            ...
            throw CloudError.uploadFailed(
                statusCode: httpResponse.statusCode,
                message: "Multipart upload failed: \(body)",
            )
        }
