...
final class GoogleDriveOAuthService: @unchecked Sendable {
    ...
    private func startListener() async throws -> UInt16 {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            ...
            queue.asyncAfter(deadline: .now() + 120.0) { [weak self] in
                self?.continuation?
                    .resume(throwing: CloudError.signingFailed("Authorization timed out after 120 seconds"))
                self?.stopListener()
            }
        }
    }
    ...
    private func exchangeCodeForTokens(
        ...
        guard let refreshToken = tokenResponse.refreshToken else {
            throw CloudError
                .signingFailed("No refresh token received. Ensure prompt=consent or access_type=offline is set.")
        }
