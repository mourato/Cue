...
//
    ...
    final class VideoEditorWindowController: NSWindowController, NSWindowDelegate {
        ...
        func showWindow() {
            ...
            window?.installTrafficLightsLayout()
        }
            ...
            Task {
                do {
                    ...
                    if state.isGIF {
                        ...
                        try await VideoEditorExporter
                            .exportTrimmed(state: state, to: destinationURL) { [weak self] progress in
                                Task { @MainActor in
                                    self?.state?.exportProgress = progress
                                    self?.state?.exportStatusMessage = self?.progressMessage(for: progress) ?? L10n
                                        .VideoEditor.exporting
                                }
                            }
                    }
            ...
            Task {
                do {
                    try GIFResizer.resize(
                        ...
                        Task { @MainActor in
                            state.exportProgress = Float(progress)
                            state.exportStatusMessage = progress < 0.95 ? L10n.VideoEditor.resizingFrames : L10n
                                .VideoEditor
                                .finalizing
                        }
                    }
            ...
            Task {
                do {
                    try await VideoEditorExporter.replaceOriginal(state: state) { [weak self] progress in
                        Task { @MainActor in
                            self?.state?.exportProgress = progress
                            self?.state?.exportStatusMessage = self?.progressMessage(for: progress) ?? L10n.VideoEditor
                                .exporting
                        }
                    }
                    state.isExporting = false
