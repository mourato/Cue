...
@MainActor
private final class ProgressUpdates {
    var values: [Double] = []
}
...
final class S3MultipartUploaderTests: XCTestCase {
    ...
    func testMultipartUpload_success() async throws {
        ...
        let progressUpdates = ProgressUpdates()
        let result = try await uploader.upload(
            ...
            Task { @MainActor in
                progressUpdates.values.append(progress)
            }
        }
        ...
        let recordedProgress = await progressUpdates.values
